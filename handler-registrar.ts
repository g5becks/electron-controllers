...
const mergeHandlers = (controllers?: Set<IpcController>, handlers?: Set<IpcHandler<any, any>>): Set<IpcHandler<any, any>> => {
...
export const registerHandlers = (controllers?: IpcController[], handlers?: IpcHandler<any, any>[]):void => {
    mergeHandlers(new Set(controllers), new Set(handlers)).forEach(handler => ipcMain.on(handler.channel, (event, req) => handler.handle(event, req)))
}
